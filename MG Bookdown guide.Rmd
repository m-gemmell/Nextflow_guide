...
A compass (
```{r, echo=FALSE}
icons::fontawesome("compass", style="solid")
```
...
Add the following to your `style.css` page. You can of course change the coulors but I find the below one is good.
...
I have used it to change the font-family of code. I am using the font-family "JetBrains Mono" from google fonts (https://fonts.google.com/specimen/JetBrains+Mono?preview.text=1%20l).
...
For this I add the following to the top of my `style.css` code:

```{css, eval=FALSE}
@import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@600&display=swap');
```

The I change the code bit in `stle.css` to:
